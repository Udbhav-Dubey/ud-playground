...
bool read_file(const std::string& filename, std::string& out, bool binary = false) {
    if (filename.empty()) return false;  // Guard against empty filename
...
int main() {
    ...
    std::cout << "Server now listening on port: " << port << "\n";
    ...
    while (true) {
        ...
        if (read_file(fileToSend, data, binary)) {
            send_200(client_sock, contentType, data);
            std::cout << "Served: " << fileToSend << " (" << data.size() << " bytes)\n";
        } else {
            send_404(client_sock);
            std::cout << "404: File not found or invalid route\n";
        }
